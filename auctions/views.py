...
def view_listing(request, id):
    ...
    if request.user.is_authenticated:
        ...
        if request.user in listing.watchlist.all(): # Check if user has already added listing to watchlist
            addedtowatchlist = True

        if Bid.objects.filter(bidder=request.user).exists(): # If the user has already placed a bid
            if (Bid.objects.get(bidder=request.user).bid_amount == current_highest_bid): # If the user has the highest bid
                viewmessage = "Your bid is the current bid."
        
        if listing.close_listing == True:# Listing was  already closed
            if Bid.objects.filter(listing_id=id).exists():
                highest_bidder = Bid.objects.get(bid_amount=current_highest_bid).bidder
                if highest_bidder == request.user: #user has won the bidding
                    winmessage = "You won!"
                else:
                    winmessage = f'{highest_bidder} has won!'
        ...
        if request.method == 'POST': 
            ...
            if form.is_valid():
                ...
                if (current_bid > listing.start_amount and current_bid > highest_bid):
                    if Bid.objects.filter(bidder=request.user).exists(): #update bid if user has alreadt placed bid previously
                        obj= Bid.objects.get(bidder=request.user)
                    else:
                        obj = Bid()
                    obj.listing = listing
